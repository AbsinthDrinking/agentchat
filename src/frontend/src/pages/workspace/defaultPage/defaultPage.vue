...
const handleSend = async () => {
  ...
  // 根据模式跳转到不同的页面
  if (selectedMode.value === 'smart-search') {
    // 智能搜索模式：直接跳转到任务流程图页面（三列布局）
    console.log('跳转到智能搜索任务页面')
    ...
    <div class="chat-container">
      ...
      <div v-if="messages.length === 0" class="welcome-section">
        <div class="avatar-wrapper">
          <img src="../../../assets/robot.svg" alt="智能小助手" class="avatar" />
        </div>
        <h1 class="welcome-title">我是智能小助手，很高兴见到你！</h1>
        <p class="welcome-subtitle">
          欢迎体验智能小助手，一位懂得完成复杂任务的 Agent 助理~
        </p>
      </div>
      ...
      <div class="input-section" :class="{ 'input-fixed': messages.length > 0 }">
        <div class="input-wrapper" :class="{ 'smart-search-glow': selectedMode === 'lingseek' }">
          <textarea
            v-model="inputMessage"
            placeholder="给智能小助手发消息，让小助手帮你完成任务~"
            class="message-input"
            rows="4"
...
.mode-selector {
  ...
  .mode-btn {
    ...
    &.active {
      border-color: var(--accent);
      background: linear-gradient(135deg, var(--accent-glow) 0%, rgba(88, 166, 255, 0.1) 100%);
...
// 智能搜索模式输入框外发光“呼吸”动画（淡蓝色，颜色不变，仅强弱变化）
@keyframes search-breath {
  0%, 100% {
    box-shadow:
...
@keyframes search-breath-strong {
  0%, 100% {
    box-shadow:
...
.input-section {
  ...
  .input-wrapper {
    ...
    &.smart-search-glow {
      ...
      animation: search-breath 2.8s ease-in-out infinite;

      &:focus-within {
        border-color: rgba(102, 126, 234, 0.55);
        animation: search-breath-strong 2.2s ease-in-out infinite;
        transform: translateY(-2px);
      }
    }
