...
async def _update_exist_llm():
    ...
    - 若无变化 → 跳过
    - 若 API Key 是掩码（包含 **）→ 保留数据库中的 key，但仍更新 model/base_url
    """
    ...
    # API Key 含掩码时保留数据库中的真实 key
    api_key_masked = api_key and "**" in api_key
    effective_api_key = llm.api_key if api_key_masked else api_key
    ...
    if api_key_masked:
        logger.success(f"LLM config updated (model={model}, base_url={base_url}, api_key unchanged)")
    else:
